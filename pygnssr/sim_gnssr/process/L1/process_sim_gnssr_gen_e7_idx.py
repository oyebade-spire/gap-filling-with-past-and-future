...
def sim_gnssr_latlon2e7_idx(file_in, dir_out, grid_res=3000, overwrite=False):
    """
    This program retrieves the Specular Point (SP) locations from sim_gnssr netCDF file, searches for
    corresponding equi7grid point and stores the indices as Python object

    :param file_in: sim_gnssr Level-1 netCDF data file
    ...
    try:
        file_out = os.path.join(dir_out, os.path.basename(file_in) + '__e7indices'+'_'+str(grid_res)+'m.json')
        ...
        if os.path.exists(file_out) and not overwrite:
            ...
            # read SP lon/lat arrays from sim_gnssr netCDF file
...
def _wrapper(dir_in, dir_out, grid_res=3000, mp_num=1, overwrite=False, node=None):
    """
    This is a wrapper code for reading sim_gnssr files in given directory, searching for the nearest equi7grid points
    corresponding sim_gnssr observations and storing the indices as python object files in the given output directory

    :param dir_in: Input directory of sim_gnssr netCDF data files
    ...
    files_in = np.array(glob.glob(os.path.join(dir_in, "sched_*.nc")))
    if node is not None:
        a = (node-1) * 70
    ...
    if len(files_in) == 0:
       return False
    sub_dir_out = os.path.join(dir_out, "G"+str(grid_res)+"M")
    files_out = np.array([os.path.join(sub_dir_out, os.path.basename(x) +
    ...
    if not overwrite:
        for i, f in enumerate(files_out):
            if not os.path.exists(f):
                filter_idx.append(i)
        if len(filter_idx) == 0:
            print(" data files were already processed (exist in output directory)!")
            return True
    ...
    partial_func = partial(sim_gnssr_latlon2e7_idx, dir_out=sub_dir_out, grid_res=grid_res)
    not_proc_files = mp.Pool(processes=mp_num).map(partial_func, files_in)
...
@click.option('-res', default=1000, type=int,
...
def main(date, days, src, dst, res, mp_num):
    """
    This program finds the nearest Equi7Grid to sim_gnssr observations and stores the results as indices in json file format.

    """
    res = 1000
    node = 1
    print(datetime.now(), " Data indexing started from python code ...")
    dir_in = os.path.join(src, "temp_working_dir", "2020-09-17_gnss-r_coverage_maps", "schedules_att90_netcdf_int", "schedules_att90_netcdf_int_10_hz")
    dir_out = os.path.join(dst, "datacube", "sim_gnssr_10hz", "sim_gnssr_e7_indices")
    os.makedirs(dir_out, exist_ok=True)
    _wrapper(dir_in, dir_out, grid_res=res, mp_num=mp_num, node=node)
